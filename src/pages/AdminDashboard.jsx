...
export default function AdminDashboard() {
  ...
  if (loading) {
    return (<div style={{minHeight:'100vh',background:'#f3f4f6',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Arial,sans-serif'}}><p style={{fontSize:'18px'}}>Loading Admin Panel...</p></div>);
  }

  if (error) {
    return (
      <div style={{minHeight:'100vh',background:'#f3f4f6',display:'flex',alignItems:'center',justifyContent:'center',padding:'32px',fontFamily:'Arial,sans-serif'}}>
        <div style={{background:'#dc2626',color:'white',padding:'24px',borderRadius:'8px',maxWidth:'500px',width:'100%',textAlign:'center'}}>
          <h2 style={{margin:'0 0 12px 0',fontSize:'20px'}}>⚠️ ERROR</h2>
          <p style={{fontSize:'14px',fontFamily:'monospace',wordBreak:'break-word',margin:'0'}}>{error}</p>
        </div>
      </div>
    );
  }
