...
export default function AdminDashboard() {
  ...
  const addCandidate = () => {
    if(name && position) {
      const newCandidates = [...candidates, { id: Date.now(), name, position, dept, votes: 0 }];
      ...
      {tab === 'results' && (
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <table className="w-full border">
            <thead><tr className="bg-gray-100"><th className="p-3 border text-left">S/N</th><th className="p-3 border text-left">Candidate Name</th><th className="p-3 border text-left">Position</th><th className="p-3 border text-left">Department</th><th className="p-3 border text-left">Votes</th></tr></thead>
            <tbody>
              {candidates.sort((a,b) => b.votes - a.votes).map((c, index) => (
                <tr key={c.id}><td className="p-3 border">{index + 1}</td><td className="p-3 border">{c.name}</td><td className="p-3 border">{c.position}</td><td className="p-3 border">{c.dept}</td><td className="p-3 border font-bold">{c.votes}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
